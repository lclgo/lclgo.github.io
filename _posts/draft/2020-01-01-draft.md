...
`perf record -g -p PID -- sleep 20`，`perf top -p PID -g -d 100`（-d表示多久刷新一次数据）
...
`perf stat -e dTLB-loads,dTLB-load-misses,iTLB-loads,iTLB-load-misses ./test`，`perf stat -p 1 -e cache-reference -e cache-misses`
